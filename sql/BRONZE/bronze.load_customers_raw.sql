...
CREATE OR ALTER PROCEDURE bronze.load_customers_raw
AS
BEGIN
    DECLARE 
        @start_time DATETIME, 
    ...
    BEGIN TRY
        ...
        PRINT '>> Truncating Table: bronze.customers_raw';
        TRUNCATE TABLE bronze.customers_raw;

        PRINT '>> Inserting Data Into: bronze.customers_raw';

        BULK INSERT bronze.bookings_raw
        FROM 'C:\Users\MILKA\Documents\Hospitality-data-warehouse\data\dim_customer.csv'
        WITH (
            FIRSTROW = 2,
            FIELDTERMINATOR = ',',
            ROWTERMINATOR = '\n',
